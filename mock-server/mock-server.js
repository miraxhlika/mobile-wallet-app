...
function genMockTransactions() {
  ...
  const wallets = [
    { wallet_id: 1, currency_id: 1 }, // USD
    { wallet_id: 2, currency_id: 2 }, // EUR
    { wallet_id: 3, currency_id: 9 }, // GBP
  ];

  const types = ["top-up", "withdrawal", "fee"];
  ...
  const total = 160; // enough for multiple pages across filters

  const out = [];

  for (let i = 0; i < total; i++) {
    const w = pick(rng, wallets);
    ...
    if (type === "top-up") {
      reason = pick(rng, reasonsTopUp);
      amount = cents(20 + rng() * 3000);
    } else if (type === "withdrawal") {
      reason = pick(rng, reasonsWithdrawal);
      amount = -cents(5 + rng() * 1200);
    } else {
      // fee
      reason = pick(rng, reasonsFee);
      amount = -cents(0.5 + rng() * 25);
    }

    out.push({
      // Stable id for easier testing
      id: 10_000 + i,
  ...
  // Keep a few human-readable seeds near the top of the feed (newest dates)
  out.push(
    {
      id: 9001,
      wallet_id: 1,
...
const mockTransactions = genMockTransactions();
...
app.get("/transactions", authenticateToken, (req, res) => {
  ...
  // Apply filters
  if (wallet_id) {
    const walletIds = toList(wallet_id).map((x) => parseInt(x));
